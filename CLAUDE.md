...
- **Vendored Dependencies**: Do NOT edit `lib/` directly; refresh native wheels via `update_grpc.py`, `update_miniaudio.py`, `update_cffi.py`[cite: 3]. `bin/dengjen-tts-grpc.exe` is not committed: run `python update_dengjen_tts.py fetch` before `scons`/`tests_contract/`; re-pin `dengjen-tts.lock` with `update_dengjen_tts.py bump` (`update-dengjen-tts.yml` opens the bump PR daily, on dispatch, or manually; it skips when a PR for that version exists). gRPC stubs in `adapters/dengjen_grpc/grpc_protos/` are generated by `update_grpc_protos.py` from the release pinned in `dengjen-tts.lock` (needs `grpcio-tools==1.62.3` and Python 3.12 or older: it has no 3.13 wheel); do not hand-edit.
