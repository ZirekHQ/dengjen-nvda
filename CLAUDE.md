...
- **Core Subsystems & Data Flow**:
  ...
  - `adapters/sonata_grpc/`: the `TTSBackend` adapter; spawns detached `dengjen-tts-grpc.exe` subprocess on a free port, explicitly validates `vcruntime140_1.dll` presence prior to spawn[cite: 3].
