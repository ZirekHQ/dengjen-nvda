...
>
> Этот перевод может отставать от [readme на английском языке](https://github.com/austek/sonata-nvda/blob/main/readme.md).
...
Вы можете найти пакет дополнения в разделе assets на [странице релиза](https://github.com/austek/sonata-nvda/releases/latest)
...
Откройте менеджер голосов из главного меню NVDA, пункт `Менеджер голосов Sonata...`. В нём две вкладки: `Загрузить` и `Установленные`.
...
Когда в качестве синтезатора выбран `Sonata Neural Voices`, следующие параметры появляются в настройках речи NVDA (`меню NVDA` > `Параметры` > `Настройки` > `Речь`).
...
**Sonata отсутствует в списке синтезаторов NVDA или не загружается.** Две обычные причины - отсутствие пакета Visual C++, описанного выше в разделе Требования, и отсутствие установленных голосов: драйвер намеренно отказывается загружаться, когда не может найти хотя бы один голос. Откройте менеджер голосов из главного меню NVDA, установите голос и перезапустите NVDA.
...
Пожалуйста, сообщайте об ошибках и запросах новых возможностей в [трекере задач этого форка](https://github.com/austek/sonata-nvda/issues), прилагая журнал вместе с версией NVDA и голосом, который вы использовали.
